...
def plot_solution_and_fourier(times,path, eps_name):
    ...
    [ax0[k].plot(list(range(len(fourier_deeponet[:50]))),[f[modes[k]-1] for f in fourier_deeponet[:50]],color=colors[k], label=str(modes[k])) for k in range(3)]
    ax0[0].set_title('mode=1');ax0[1].set_title('mode=6');ax0[2].set_title('mode=10');
