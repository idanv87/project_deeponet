...
def generate_data(names,  save_path, number_samples,seed=0):
    ...
    for l,name in enumerate(names):
        ...
        M=domain['M']
        angle_fourier=domain['angle_fourier'][:90]
...
if __name__=='__main__':
    ...
    X,Y=generate_data(train_names, Constants.train_path, number_samples=200, seed=0)
    X_test, Y_test=generate_data(test_names,Constants.test_path,number_samples=1, seed=4)

    print('finished data generation')

if False: 
    test_files=extract_path_from_dir(Constants.test_path)
    ...
    train_files=extract_path_from_dir(Constants.train_path)
